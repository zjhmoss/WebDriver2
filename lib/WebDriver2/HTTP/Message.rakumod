unit class WebDriver2::HTTP::Message;

use WebDriver2::HTTP::Header;
use WebDriver2::HTTP::MediaType;
use Encode;

has WebDriver2::HTTP::Header $.header = WebDriver2::HTTP::Header.new;
has $.content is rw;
...
has WebDriver2::HTTP::MediaType $!media-type;

method media-type() returns WebDriver2::HTTP::MediaType {
    if not $!media-type.defined { 
        if self.content-type() -> $ct {
            $!media-type = WebDriver2::HTTP::MediaType.parse($ct);
        }
    }
    $!media-type;
}

# Don't want to put the heuristic in the WebDriver2::HTTP::MediaType
...
method Str($eol = "\n", :$debug, Bool :$bin) {
    ...
    if not $bin {
        ...
        $s ~=  $.content if $.content and !$debug || $debug < 2;
    }
    if $.content and $debug and $debug == 1 | 3 {
...
WebDriver2::HTTP::Message - class encapsulating HTTP message

=head1 SYNOPSIS

    use WebDriver2::HTTP::Message;
    my $raw_msg = "GET / HTTP/1.1\r\nHost: somehost\r\n\r\n";
    my $mess = WebDriver2::HTTP::Message.new.parse($raw_msg);
    say $mess;
...
=item fields : fields of the header section
    ...
    my $msg = WebDriver2::HTTP::Message.new('content', :field<value>);

=head2 method add-content

    method add-content(WebDriver2::HTTP::Message:, Str $content)
...
=head2 method decoded-content
    ...
    method decoded-content(WebDriver2::HTTP::Message:)

Returns decoded content of the message (using L<Encode> module to decode).

    my $msg = WebDriver2::HTTP::Message.new();
    say $msg.decoded-content;

=head2 method field

    multi method field(WebDriver2::HTTP::Message:, Str $s) returns WebDriver2::HTTP::Header::Field
    multi method field(WebDriver2::HTTP::Message:, *%fields)

See L<WebDriver2::HTTP::Header>.

=head2 method init-field

    method init-field(WebDriver2::HTTP::Message:, *%fields)

See L<WebDriver2::HTTP::Header>.

=head2 method push-field

    method push-field(WebDriver2::HTTP::Message:, WebDriver2::HTTP::Header::Field $field)

See L<WebDriver2::HTTP::Header>.

=head2 method remove-field

    method remove-field(WebDriver2::HTTP::Message:, Str $field)

See L<WebDriver2::HTTP::Header>.

=head2 method clear

    method clear(WebDriver2::HTTP::Message:)
...
=head2 method parse
    ...
    method parse(WebDriver2::HTTP::Message:, Str $raw_message) returns WebDriver2::HTTP::Message

Parses the whole HTTP message.

It takes the HTTP message (with \r\n as a line separator)
and obtain the header and content section, creates a WebDriver2::HTTP::Header
object.

    my $msg = WebDriver2::HTTP::Message.new.parse("GET / HTTP/1.1\r\nHost: example\r\ncontent\r\n");
    say $msg.perl;

=head2 method Str

    method Str(WebDriver2::HTTP::Message:, Str $eol = "\n") returns Str

Returns HTTP message in a readable form.

=head1 SEE ALSO

L<WebDriver2::HTTP::Request>, L<WebDriver2::HTTP::Response>, L<Encode>
