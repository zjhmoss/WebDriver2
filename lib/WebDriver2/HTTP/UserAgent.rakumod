...
has $.useragent;
has WebDriver2::HTTP::Cookies $.cookies is rw = WebDriver2::HTTP::Cookies.new(
    file     => tempfile[0],
    autosave => 1,
);
has $.auth_login;
...
submethod BUILD(:$!useragent, Bool :$!throw-exceptions, :$!max-redirects = 5, Int :$!debug = 0, :$!redirects-in-a-row) {
    $!useragent = get-ua($!useragent) if $!useragent.defined;
    if $!debug.defined {
        if $!debug ~~ Int and $!debug > 1 {
            $!debug-handle = $*OUT;
        }
        if $!debug ~~ Str {
            say $!debug;
...
multi method get(URI $uri is copy, Bool :$bin,  *%header ) {
    my $request  = WebDriver2::HTTP::Request.new(GET => $uri, |%header);
...
multi method post(URI $uri is copy, %form , Bool :$bin,  *%header) {
    my $request = WebDriver2::HTTP::Request.new(POST => $uri, |%header);
...
multi method put(URI $uri is copy, %form , Bool :$bin,  *%header) {
    my $request = WebDriver2::HTTP::Request.new(PUT => $uri, |%header);
    $request.add-form-data(%form);
    self.request($request, :$bin);
}
...
multi method delete(URI $uri is copy, Bool :$bin,  *%header ) {
    my $request  = WebDriver2::HTTP::Request.new(DELETE => $uri, |%header);
    self.request($request, :$bin);
}
...
method request(WebDriver2::HTTP::Request $request, Bool :$bin) returns WebDriver2::HTTP::Response {
    my WebDriver2::HTTP::Response $response;
    ...
    $.debug-handle.say("==>>Send\n" ~ $request.Str(:debug)) if $.debug and $.debug == 1 | 3;
    ...
    X::WebDriver2::HTTP::Response.new(:rc('No response')).throw unless $response;

    $.debug-handle.say("<<==Recv\n" ~ $response.Str(:debug)) if $.debug and $.debug == 1 | 3;
    ...
    if $response.code ~~ /^30<[0123]>/ {
        ...
        if $.max-redirects < $.redirects-in-a-row {
            X::WebDriver2::HTTP::Response.new(:rc('Max redirects exceeded'), :response($response)).throw;
        }
    ...
    if $!throw-exceptions {
        given $response.code {
            when /^4/ {
                X::WebDriver2::HTTP::Response.new(:rc($response.status-line), :response($response)).throw;
            }
            when /^5/ {
                X::WebDriver2::HTTP::Server.new(:rc($response.status-line), :response($response)).throw;
            }
        }
    }
...
method get-response(WebDriver2::HTTP::Request $request, Connection $conn, Bool :$bin) returns WebDriver2::HTTP::Response {
    ...
    CATCH {
        ...
        when /'Connection reset by peer'/ {
            X::WebDriver2::HTTP::Internal.new(rc => 500, reason => "Connection reset by peer").throw;
        }
    }
    ...
    my WebDriver2::HTTP::Response $response = WebDriver2::HTTP::Response.new($header-chunk);
    ...
    if $response.has-content {
        ...
        CATCH {
            when X::WebDriver2::HTTP::ContentLength {
                X::WebDriver2::HTTP::Header.new( :rc($_.message), :response($response) ).throw
            }
        }
...
multi method get-connection(WebDriver2::HTTP::Request $request ) returns Connection {
...
method get-proxy(WebDriver2::HTTP::Request $request) {
...
multi method use-proxy(WebDriver2::HTTP::Request $request) returns Bool {
...
method setup-auth(WebDriver2::HTTP::Request $request) {
    # use HTTP Auth
...
method use-auth(WebDriver2::HTTP::Request $request) {
    $!auth_login.defined && $!auth_password.defined;
}

# :simple
our sub get($target where URI|Str) is export(:simple) {
    my $ua = WebDriver2::HTTP::UserAgent.new(:throw-exceptions);
...
our sub getprint(Str $url) is export(:simple) {
    my $response = WebDriver2::HTTP::UserAgent.new(:throw-exceptions).get($url);
...
WebDriver2::HTTP::UserAgent - Web user agent class

=head1 SYNOPSIS

    use WebDriver2::HTTP::UserAgent;

    my $ua = WebDriver2::HTTP::UserAgent.new;
    $ua.timeout = 10;
...
=head2 method new
    ...
    method new(WebDriver2::HTTP::UserAgent:U: :$!useragent, Bool :$!throw-exceptions, :$!max-redirects = 5, :$!debug) returns WebDriver2::HTTP::UserAgent
...
L<WebDriver2::HTTP::UserAgent::Common>, but a string that is not specified there will be
used verbatim.
...
L<WebDriver2::HTTP::Response> returned.  If this is specified then an exception will
...
=head2 method auth
    ...
    method auth(WebDriver2::HTTP::UserAgent:, Str $login, Str $password)

Sets username and password needed to HTTP Auth.

=head2 method get

    multi method get(Str $url is copy, :bin?, *%headers) returns WebDriver2::HTTP::Response
    multi method get(URI $uri, :bin?, *%headers) returns WebDriver2::HTTP::Response

Requests the $url site, returns WebDriver2::HTTP::Response, except if throw-exceptions
...
=head2 method post
    ...
    multi method post(URI $uri, %form, *%header ) -> WebDriver2::HTTP::Response
    multi method post(Str $uri, %form, *%header ) -> WebDriver2::HTTP::Response
...
An L<WebDriver2::HTTP::Response> will be returned, except if throw-exceptions has been set
...
create an L<WebDriver2::HTTP::Request> directly and populate it as needed,

=head2 method request

    method request(WebDriver2::HTTP::Request $request, :bin?) returns WebDriver2::HTTP::Response

Performs the request described by the supplied L<WebDriver2::HTTP::Request>, returns
a L<WebDriver2::HTTP::Response>, except if throw-exceptions is set as described above
...
You can use the helper subroutines defined in L<WebDriver2::HTTP::Request::Common> to
create the L<WebDriver2::HTTP::Request> for you or create it yourself if you have more
