unit class WebDriver2::HTTP::Request::Common;
...
my sub get-request(Str $meth, URI $uri, Bool :$bin, *%headers) returns WebDriver2::HTTP::Request {
    my $request  = WebDriver2::HTTP::Request.new(|($meth.uc => $uri), :$bin);
    $request.header.field(|%headers);
    $request;
}

my sub send-text-content(Str $meth, URI $uri, :$content, *%headers is copy ) returns WebDriver2::HTTP::Request {
...
WebDriver2::HTTP::Request::Common - Construct common WebDriver2::HTTP::Request objects

=head1 SYNOPSIS

    use WebDriver2::HTTP::Request::Common;
...
This module provide functions that return newly created "WebDriver2::HTTP::Request"
objects. These functions are usually more convenient to use than the
standard "WebDriver2::HTTP::Request" constructor for the most common requests. The
following functions are provided:

=head2 C<GET $url, Header => Value...>

The GET() function returns an C<WebDriver2::HTTP::Request> object initialized with
